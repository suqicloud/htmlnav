...
$(function(){
    // 初始状态下显示侧边栏导航
    $('#type-left').addClass('showListType');
    ...
    // 点击文档其他区域时隐藏侧边栏导航（如果已关闭）
    $(document).click(function(e){
        var con = $('.type-left');
        // 判断侧边栏导航当前是否已关闭
        if (!con.hasClass('showListType')) {
            // 如果已关闭，则保持关闭状态，不做任何操作
            return;
        }
        // 如果未关闭，则显示侧边栏导航
        con.addClass('showListType');
    });
...
/*$(function(){
    ...
    // 点击文档任意区域时切换侧边栏导航显示状态
    $(document).click(function(){
        clickCount++;
        if (clickCount % 2 === 1) {
            $('#type-left').removeClass('showListType');
            sidebarVisible = false;
        } else { // Even click counts
            $('#type-left').addClass('showListType');
            sidebarVisible = true;
        }
    });
    ...
    // 点击侧边栏导航项时的操作
    $('.type-left ul li').click(function(){
        // Your scroll and active state logic here
        $(this).addClass('active').siblings('li').removeClass('active');
        var lylme_tag = '#' + $(this).attr("data-lylme");
        $('html,body').animate({scrollTop:$(lylme_tag).offset().top},500);
    });

    // 初始状态下显示侧边栏导航（可选，根据需求）
    if (sidebarVisible) {
        $('#type-left').addClass('showListType');
    } else {
        $('#type-left').removeClass('showListType');
